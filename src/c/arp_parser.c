...
int arp_parse(const uint8_t* data, uint32_t length, arp_header_t* out_header) {
    ...
    /* 16-bit fields are assembled from two big-endian bytes rather than
       read through a uint16_t*, so the result doesn't depend on host
       byte order. */
    uint16_t htype =
        (uint16_t)((data[0] << 8) | data[1]); /* link-layer type, must be 1 (Ethernet) */
    uint16_t ptype = (uint16_t)((data[2] << 8) | data[3]); /* L3 protocol, must be 0x0800 (IPv4) */

    uint8_t hlen = data[4]; /* hardware address length, must be 6 (MAC) */
    uint8_t plen = data[5]; /* protocol address length, must be 4 (IPv4) */
