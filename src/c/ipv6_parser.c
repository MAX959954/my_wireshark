...
int ipv6_parse(const uint8_t* data, uint32_t length, ipv6_header_t* out_header,
    ...
    /* Simpler than IPv4: no IHL, so no "header_len > length" check -
    ...
    /* Traffic Class straddles two bytes: low nibble of data[0] (its high
       half) and high nibble of data[1] (its low half). */
    out_header->traffic_class = (uint8_t)(((data[0] & 0x0F) << 4) | (data[1] >> 4));
    out_header->flow_label =
        ((uint32_t)(data[1] & 0x0F) << 16) | ((uint32_t)data[2] << 8) | data[3];
...
/* 16 bytes -> 8 hextets, printed in standard "xxxx:...:xxxx" hex form. */
void ipv6_addr_to_str(const uint8_t addr[IPV6_ADDR_LEN], char* output) {
    /* IPV6_ADDR_STR_LEN is sized exactly for this format string, so this can
       never truncate - the return value isn't worth checking (cert-err33-c). */
    (void)snprintf(output, IPV6_ADDR_STR_LEN, "%x:%x:%x:%x:%x:%x:%x:%x", (addr[0] << 8) | addr[1],
                   (addr[2] << 8) | addr[3], (addr[4] << 8) | addr[5], (addr[6] << 8) | addr[7],
                   (addr[8] << 8) | addr[9], (addr[10] << 8) | addr[11], (addr[12] << 8) | addr[13],
                   (addr[14] << 8) | addr[15]);
}
